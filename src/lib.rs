...
pub fn gen(h: usize, w: usize) -> Vec<i32> {
    gen_inner(h, w).iter().map(|&f| f as i32).collect()
}

fn gen_inner(h: usize, w: usize) -> Vec<bool> {
    let mut rng = rand::thread_rng();
    loop {
        let mut board = Board::new(h, w, (0..h * w).map(|_| Some(rng.gen_bool(0.5))).collect());
        let mut solver = Solver::new(h, w, board.get_hints());
        if solver.solve() {
            return solver.board.get_plain();
        }
    }
}
...
impl Board {
    ...
    fn get_hints(&mut self) -> (Vec<Vec<usize>>, Vec<Vec<usize>>) {
...
struct Solver {
    hints: (Vec<Vec<usize>>, Vec<Vec<usize>>),
    board: Board,
}
impl Solver {
    fn new(h: usize, w: usize, hints: (Vec<Vec<usize>>, Vec<Vec<usize>>)) -> Self {
        Self {
            hints,
